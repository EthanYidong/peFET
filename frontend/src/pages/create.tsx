...
export default function Create() {
  ...
  return (
    <div class="flex flex-col p-8 gap-2">
      <div>
        <Link class="bg-blue-600 text-white p-2 rounded-md" href="/">
          &lt; Home
        </Link>
      </div>
      <h1 class="text-2xl">Create New Event Listing</h1>
      <h2 class="text-xl">
        If you're seeing this, that means you have successfully logged in. This
        page does not affect anything. It is only to demonstrate that the user
        is logged in, and serve as a prototype for event creation.
      </h2>
      <div>
        <ul>
            ...
            {(pt, idx) => (
              <li class="my-2">
                <ParticipantEditor
                  participant={pt}
                  edit={(propName, value) =>
                    setParticipants(idx(), propName, value)
                  }
                />
              </li>
            )}
      ...
      <div class="flex">
        <button
          class="button bg-blue-600 rounded-md w-16 text-white m-2"
          onClick={addButtonOnClick}
        >
          +
        </button>
        <button
          class="button bg-green-600 rounded-md w-16 text-white m-2"
